                             ...
                             virCaps *caps)
{
    ...
    /* CH supports multiple hugespages sizes but it expects exact memory to be
     * allocated in the form of memory-zones
     * Support single hugepage size for now
     */
    if (mem->nhugepages > 1) {
        virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
    ...
    /* check if the host can support requested hugepage size */
    ...
    if (i == caps->host.nPagesSize){
        virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                       _("Host does not support HugePage size %llu B"),
                        hugepage_size);
        return -1;
    }
