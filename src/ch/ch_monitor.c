...
virCHMonitorBuildNetJson(virDomainNetDef *net, char **jsonstr)
{
    ...
    if (actualType == VIR_DOMAIN_NET_TYPE_ETHERNET) {
        ...
        /*
         * else case for below is already checked in
         * virCHDomainValidateActualNetDef. No need to check again.
         */
        if (net->guestIP.nips == 1) {
            const virNetDevIPAddr *ip = net->guestIP.ips[0];
    ...
    /* Although, cloud-hypervisor accepts interface name("tap") in net json, do
     * not ever send it.
     * Cloud-hypervisor, by default, tries to create a new tap device with the
     * provided name, which conflicts with the one created by libvirt.
     */
    ...
    if (net->driver.virtio.rx_queue_size || net->driver.virtio.tx_queue_size) {
        ...
        if (virJSONValueObjectAppendNumberInt(net_json,
                            "queue_size", net->driver.virtio.rx_queue_size) < 0)
            return -1;
    }
