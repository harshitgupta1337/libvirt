...
static virCHMonitor *virCHProcessConnectMonitor(virCHDriver *driver,
                                                virDomainObj *vm) {
 ...
 */
static int chProcessNetworkPrepareDevices(virCHDriver *driver,
                                          virDomainObj *vm) {
  virDomainDef *def = vm->def;
  size_t i;
  virCHDomainObjPrivate *priv = vm->privateData;
  g_autoptr(virConnect) conn = NULL;
  g_autofree int *tapfd = NULL;
  size_t tapfdSize = 0;
  ...
  for (i = 0; i < def->nnets; i++) {
    ...
    if (actualType == VIR_DOMAIN_NET_TYPE_HOSTDEV &&
      ...
      if (virDomainHostdevFind(def, hostdev, NULL) >= 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       _("PCI device %04x:%02x:%02x.%x "
                         ...
                         "in use by domain %s"),
                       pcisrc->addr.domain, pcisrc->addr.bus, pcisrc->addr.slot,
                       pcisrc->addr.function, net->data.network.name,
                       def->name);
        return -1;
      }
      ...
      if (g_alloc(tapfd, tapfdSize) < 0) < 0)
        goto cleanup;

      memset(tapfd, -1, tapfdSize * sizeof(tapfd[0]));

      if (chInterfaceBridgeConnect(def, driver, net, tapfd, &tapfdSize) < 0)
        return -1;
